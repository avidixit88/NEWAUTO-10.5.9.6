...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    # accept line priority (preserve the current v83b behavior as the primary fallback)
    if direction == "LONG":
        if vwap_reclaim and ref_vwap is not None:
            primary_accept_line, primary_accept_src = float(ref_vwap), "VWAP"
        elif orb_high is not None and orb_break_up:
            primary_accept_line, primary_accept_src = float(orb_high), "ORB"
        else:
            primary_accept_line, primary_accept_src = float(ema20.iloc[-1]), "EMA20"
    else:
        if vwap_reject and ref_vwap is not None:
            primary_accept_line, primary_accept_src = float(ref_vwap), "VWAP"
        elif orb_low is not None and orb_break_dn:
            primary_accept_line, primary_accept_src = float(orb_low), "ORB"
        else:
            primary_accept_line, primary_accept_src = float(ema20.iloc[-1]), "EMA20"

    # Primary anchor: preserve the current primary impulse voting / anchor logic.
    if direction == "LONG":
        if orb_break_up and isinstance(orb_high, (float, int)):
            primary_impulse_type, primary_impulse_level = "ORB", float(orb_high)
        elif pivot_break_up and isinstance(swing_hi, (float, int)):
            primary_impulse_type, primary_impulse_level = "PIVOT", float(swing_hi)
        else:
            primary_impulse_type, primary_impulse_level = "VWAP", float(ref_vwap) if isinstance(ref_vwap, (float, int)) else float(primary_accept_line)
    else:
        if orb_break_dn and isinstance(orb_low, (float, int)):
            primary_impulse_type, primary_impulse_level = "ORB", float(orb_low)
        elif pivot_break_dn and isinstance(swing_lo, (float, int)):
            primary_impulse_type, primary_impulse_level = "PIVOT", float(swing_lo)
        else:
            primary_impulse_type, primary_impulse_level = "VWAP", float(ref_vwap) if isinstance(ref_vwap, (float, int)) else float(primary_accept_line)

    primary_impulse_idx: Optional[int] = None
    try:
        lvl = float(primary_impulse_level)
        if direction == "LONG":
        ...
        if len(cross_locs):
            primary_impulse_idx = int(cross_locs[-1])
    except Exception:
        primary_impulse_idx = None

    if direction == "LONG":
        if primary_impulse_idx is not None:
            primary_break_trigger = float(max(float(primary_impulse_level), float(df["high"].iloc[primary_impulse_idx])))
        else:
            primary_break_trigger = float(primary_impulse_level)
    else:
        if primary_impulse_idx is not None:
            primary_break_trigger = float(min(float(primary_impulse_level), float(df["low"].iloc[primary_impulse_idx])))
        else:
            primary_break_trigger = float(primary_impulse_level)

    def _detect_orb_secondary_leg() -> tuple[bool, str | None, float | None, int | None, float | None, float | None, str | None]:
        """Detect a fresh ORB-derived continuation leg by scanning backward for the most recent
        valid re-base breakout. Intentionally restricted to ORB primaries to minimize blast radius.
        Recomputed every rerun from price history (no session-state memory).
        """
        try:
            if str(primary_impulse_type).upper() != "ORB":
                return False, None, None, None, None, None, None
            if primary_impulse_idx is None or len(df) < 12:
                return False, None, None, None, None, None, None
            highs = df["high"].astype(float).to_numpy()
            lows = df["low"].astype(float).to_numpy()
            closes = df["close"].astype(float).to_numpy()
            base_n = 4 if interval_mins <= 5 else 3
            base_n = int(max(3, min(base_n, len(df) - 3)))
            start_i = int(max((primary_impulse_idx or 0) + base_n + 1, base_n + 2))
            rebreak_buf = float(max(0.10 * atr_last, 0.50 * buffer))
            ext_req = float(0.90 * atr_last)
            comp_cap = float(max(0.55 * atr_last, 0.95 * atr_last))
            found = None
            for j in range(start_i, len(df)):
                b0 = j - base_n
                if b0 <= (primary_impulse_idx or 0):
                    continue
                bh = float(np.max(highs[b0:j]))
                bl = float(np.min(lows[b0:j]))
                br = float(bh - bl)
                if br <= 0.0 or br > comp_cap:
                    continue
                if direction == "LONG":
                    prior_ext = float(np.max(highs[(primary_impulse_idx or 0):j])) - float(primary_break_trigger)
                    if prior_ext < ext_req:
                        continue
                    # Require the base to hold above the original ORB trigger area.
                    if float(np.min(closes[b0:j])) < float(primary_break_trigger) - 0.20 * atr_last:
                        continue
                    broke = bool(
                        closes[j] > bh + rebreak_buf
                        and (closes[j - 1] <= bh + 0.50 * rebreak_buf or highs[j - 1] <= bh + rebreak_buf)
                    )
                    disp_leg = bool((highs[j] - lows[j]) / max(1e-9, float(atr_last)) >= 0.70)
                    if not (broke and disp_leg):
                        continue
                    leg_level = float(bh)
                    leg_idx = int(j)
                    leg_trigger = float(max(leg_level, highs[j]))
                    leg_accept = float(leg_level)
                    found = (True, "ORB_LEG2", leg_level, leg_idx, leg_trigger, leg_accept, f"orb_rebase[{b0}:{j-1}]→break@{j}")
                else:
                    prior_ext = float(primary_break_trigger) - float(np.min(lows[(primary_impulse_idx or 0):j]))
                    if prior_ext < ext_req:
                        continue
                    if float(np.max(closes[b0:j])) > float(primary_break_trigger) + 0.20 * atr_last:
                        continue
                    broke = bool(
                        closes[j] < bl - rebreak_buf
                        and (closes[j - 1] >= bl - 0.50 * rebreak_buf or lows[j - 1] >= bl - rebreak_buf)
                    )
                    disp_leg = bool((highs[j] - lows[j]) / max(1e-9, float(atr_last)) >= 0.70)
                    if not (broke and disp_leg):
                        continue
                    leg_level = float(bl)
                    leg_idx = int(j)
                    leg_trigger = float(min(leg_level, lows[j]))
                    leg_accept = float(leg_level)
                    found = (True, "ORB_LEG2", leg_level, leg_idx, leg_trigger, leg_accept, f"orb_rebase[{b0}:{j-1}]→break@{j}")
    ...
    active_leg_found, active_leg_type, active_leg_level, active_leg_idx, active_break_trigger, leg_accept_line_raw, active_leg_reason = _detect_orb_secondary_leg()

    # Choose the currently tradeable anchor.
    if active_leg_found and active_break_trigger is not None and leg_accept_line_raw is not None:
        impulse_type = str(active_leg_type or primary_impulse_type)
        impulse_level = float(active_leg_level if active_leg_level is not None else primary_impulse_level)
        impulse_idx = int(active_leg_idx) if active_leg_idx is not None else primary_impulse_idx
        break_trigger = float(active_break_trigger)
        accept_line_raw = float(leg_accept_line_raw)
        accept_src = str(active_leg_type or "ACTIVE_LEG")
        reanchor_used = True
    else:
        impulse_type = str(primary_impulse_type)
        impulse_level = float(primary_impulse_level)
        impulse_idx = primary_impulse_idx
        break_trigger = float(primary_break_trigger)
        accept_line_raw = float(primary_accept_line)
        accept_src = str(primary_accept_src)
        reanchor_used = False

    # --- Acceptance / retest logic ---
    # NOTE: In live tape, ORB/VWAP levels can be valid but still too far from price to be a realistic pullback entry.
    # Clamp the chosen accept line into a sane ATR window around last.
    if direction == "LONG":
        lo = float(last_price - 1.20 * atr_last)
        hi = float(last_price - 0.05 * atr_last)
        accept_line = float(np.clip(float(accept_line_raw), lo, hi))
    else:
        lo = float(last_price + 0.05 * atr_last)
        hi = float(last_price + 1.20 * atr_last)
        accept_line = float(np.clip(float(accept_line_raw), lo, hi))

    # Accept = closes remain on the correct side of the chosen accept line.
    ...
    prox_atr = 0.45
    prox_dist = prox_atr * float(atr_last) + entry_pad
    if direction == "LONG":
        dist_pb_band = 0.0 if (last_price >= pb2 and last_price <= pb1) else min(abs(last_price - pb2), abs(last_price - pb1))
        stale_breakout = bool(last_price > break_trigger + 0.60 * atr_last + entry_pad and dist_pb_band > prox_dist)
    else:
        dist_pb_band = 0.0 if (last_price <= pb2 and last_price >= pb1) else min(abs(last_price - pb2), abs(last_price - pb1))
        stale_breakout = bool(last_price < break_trigger - 0.60 * atr_last - entry_pad and dist_pb_band > prox_dist)

    dist_br = abs(last_price - break_trigger)
    near_pullback = bool(dist_pb_band <= prox_dist)
    near_break = bool(dist_br <= prox_dist)
    ...
    if actionable:
        if near_pullback and (not near_break or dist_pb_band <= dist_br):
            entry_mode = "PULLBACK"
            entry_base = float(pullback_entry)
            entry_price = float(entry_base + entry_pad) if direction == "LONG" else float(entry_base - entry_pad)
            chase_line = float(break_trigger + entry_pad) if direction == "LONG" else float(break_trigger - entry_pad)
        else:
            entry_mode = "BREAKOUT"
            entry_base = float(max(break_trigger, last_price)) if direction == "LONG" else float(min(break_trigger, last_price))
            entry_price = float(entry_base + entry_pad) if direction == "LONG" else float(entry_base - entry_pad)
            chase_line = float(entry_price + 0.10 * atr_last + entry_pad) if direction == "LONG" else float(entry_price - 0.10 * atr_last - entry_pad)
            if direction == "LONG":
    ...
    _imp_lbl = str(impulse_type or "")
    if _imp_lbl.upper().startswith("ORB"):
        _imp_lbl = "ORB"
    elif _imp_lbl.upper().startswith("PIVOT"):
        _imp_lbl = "Pivot"
    elif _imp_lbl.upper().startswith("VWAP"):
        _imp_lbl = "VWAP+Disp"
    why.append("Impulse: " + _imp_lbl)
    ...
    return SignalResult(
        ...
        extras={
            ...
            "primary_impulse_type": str(primary_impulse_type),
            "primary_impulse_level": float(primary_impulse_level),
            "primary_impulse_idx": int(primary_impulse_idx) if primary_impulse_idx is not None else None,
            "primary_break_trigger": float(primary_break_trigger),
            "active_leg_found": bool(active_leg_found),
            "active_leg_type": active_leg_type,
            "active_leg_level": float(active_leg_level) if active_leg_level is not None else None,
            "active_leg_idx": int(active_leg_idx) if active_leg_idx is not None else None,
            "active_break_trigger": float(active_break_trigger) if active_break_trigger is not None else None,
            "active_leg_reason": active_leg_reason,
            "reanchor_used": bool(reanchor_used),
            "leg_accept_line": float(leg_accept_line_raw) if leg_accept_line_raw is not None else None,
            "slippage_mode": slippage_mode,

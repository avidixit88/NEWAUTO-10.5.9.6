...
def _derive_pool_reserved(state: Dict[str, Any]) -> float:
    """Authoritative pool reserve = sum(reserved_dollars) across active lifecycles."""
    total = 0.0
    try:
        ...
        for _, lst in lifecycles.items():
            for raw in (lst or []):
                if isinstance(raw, dict):
                    stg = str(raw.get("stage", "") or "").upper()
                    amt = raw.get("reserved_dollars", 0.0)
                else:
                    stg = str(getattr(raw, "stage", "") or "").upper()
                    amt = getattr(raw, "reserved_dollars", 0.0)
                if stg in {"PRESTAGED", "STAGED", "ENTRY_SENT", "IN_POSITION", "CANCEL_PENDING", "EOD_PENDING"}:
                    try:
...
def _refresh_pool_reserved(state: Dict[str, Any]) -> float:
    total = _derive_pool_reserved(state)
    try:
        state["pool_reserved"] = float(total)
    except Exception:
        pass
    return total


def _set_protection_state(lifecycle: "TradeLifecycle", state: str, *, qty: Optional[int] = None, err: Optional[str] = None) -> None:
    try:
        new_state = str(state or "").upper()
        now_iso = _now_et().isoformat()
        prev_state = str(getattr(lifecycle, "protection_state", "") or "").upper()
        prev_attempt = str(getattr(lifecycle, "protection_last_attempt_at", "") or "")
        lifecycle.protection_state = new_state
        if qty is not None:
            lifecycle.protection_required_qty = int(max(0, int(qty)))
        if err is not None:
            lifecycle.protection_last_error = str(err)[:240]
        if new_state == "LIVE":
            lifecycle.protection_fail_count = 0
            lifecycle.protection_last_attempt_at = now_iso
        elif new_state in {"FAILED", "DEGRADED"}:
            if prev_state != new_state or prev_attempt != now_iso:
                lifecycle.protection_fail_count = int(getattr(lifecycle, "protection_fail_count", 0) or 0) + 1
            lifecycle.protection_last_attempt_at = now_iso
    except Exception:
        pass
...
class TradeLifecycle:
    ...
    protection_required_qty: int = 0
    protection_last_error: Optional[str] = None
    protection_fail_count: int = 0
    protection_last_attempt_at: Optional[str] = None
    emergency_flatten_started_at: Optional[str] = None
    emergency_flatten_reason: Optional[str] = None
    eod_liquidation_started_at: Optional[str] = None
    ...
    # How many times this lifecycle has been evaluated by the reconcile loop (ENTRY_SENT/IN_POSITION/CANCEL_PENDING).
...
def lifecycle_from_raw(raw: Any) -> TradeLifecycle:
    ...
    try:
        if lc.stage not in {"PRESTAGED", "STAGED", "ENTRY_SENT", "IN_POSITION", "CLOSED", "CANCELED", "CANCEL_PENDING", "EOD_PENDING"}:
...
def _get_state() -> Dict[str, Any]:
    ...
    try:
        ...
        for _, lst in lifecycles.items():
            for raw in (lst or []):
                stg = str((raw or {}).get("stage", "")).upper()
                if stg in {"PRESTAGED", "STAGED", "ENTRY_SENT", "IN_POSITION", "CANCEL_PENDING", "EOD_PENDING"}:
                ...
                if tgt is not None:
                    for o in open_orders:
                        ...
                        if abs(float(sp) - tgt) <= _eps_for(tgt):
                            ...
                            _set_protection_state(lifecycle, "LIVE", qty=int(filled_qty), err=None)
        ...
        if not lifecycle.stop_order_id and lifecycle.stop is not None and _should_attempt('stop_last_attempt_at'):
            ...
            if _recent_submit(getattr(lifecycle, 'stop_submit_started_at', None), now, 90):
                ...
                try:
                    ...
                    emsg = str(e)
                    _set_protection_state(lifecycle, "FAILED", qty=int(filled_qty), err=emsg)
...
def _orphan_recovery_and_protection_guard(cfg, state: dict, client, account_id_key: str, now) -> None:
    ...
    # 2) Recover broker-side orphans. Prefer broker positions, but tolerate a
    # temporary positions-map miss by also scanning open broker orders for an
    # existing protective plan (STOP + LIMIT SELL) and inferring qty from it.
    _bc = state.setdefault("_broker_cache", {}) if isinstance(state, dict) else {}
    positions_map: dict = {}
    try:
        positions_map = client.get_positions_map(account_id_key) or {}
        if isinstance(_bc, dict):
            _bc["orphan_positions_map"] = {"ts": pytime.time(), "data": dict(positions_map)}
    except Exception:
        ent = _bc.get("orphan_positions_map") if isinstance(_bc, dict) else None
        if isinstance(ent, dict):
            try:
                ts = float(ent.get("ts", 0) or 0)
                if (pytime.time() - ts) <= 300:
                    positions_map = ent.get("data") or {}
            except Exception:
                positions_map = {}

    def _qty_from_pos(v) -> float:
        try:
            if isinstance(v, dict):
                return float(v.get("qty", v.get("quantity", 0)) or 0)
            return float(v or 0)
        except Exception:
            return 0.0

    # Seed candidates from positions.
    orphan_candidates: dict[str, float] = {}
    for sym, pos in (positions_map or {}).items():
        symbol = str(sym).upper().strip()
        qty = _qty_from_pos(pos)
        if symbol and qty > 0:
            orphan_candidates[symbol] = qty

    # Also scan open orders once. If we can see a full bracket plan for a symbol,
    # infer candidate qty from the SELL order size. This helps when positions-map
    # broker truth is transiently unavailable but open broker orders still reveal
    # the managed position.
    open_orders_by_symbol: dict[str, list[dict]] = {}
    try:
        od = client.list_orders(account_id_key, status="OPEN", count=200)
    ...
    for o in orders or []:
        if not isinstance(o, dict):
            continue
        symbol = str(o.get("symbol") or o.get("displaySymbol") or "").upper().strip()
        if not symbol:
            continue
        try:
            oid = int(o.get("orderId", o.get("orderID", 0)) or 0)
        ...
        qty = None
        try:
            odetail = o.get("OrderDetail", o.get("orderDetail", []))
            if isinstance(odetail, dict):
                odetail = [odetail]
            if odetail and isinstance(odetail[0], dict):
                lp = odetail[0].get("limitPrice", odetail[0].get("price"))
                sp = odetail[0].get("stopPrice")
                qv = odetail[0].get("orderedQuantity", odetail[0].get("quantity", odetail[0].get("qty")))
                limit_price = float(lp) if lp not in (None, "") else None
                stop_price = float(sp) if sp not in (None, "") else None
                qty = int(float(qv)) if qv not in (None, "") else None
        except Exception:
            pass
        if limit_price is None:
            try:
        ...
        if qty is None:
            try:
                qv = o.get("orderedQuantity", o.get("quantity", o.get("qty")))
                qty = int(float(qv)) if qv not in (None, "") else None
            except Exception:
                qty = None
    ...
    for symbol, olist in open_orders_by_symbol.items():
        if symbol in lifecycles and lifecycles.get(symbol):
            continue
        stop_orders = [o for o in olist if ("STOP" in (o.get("order_type") or "")) and (o.get("action") == "SELL")]
        tp_orders = [o for o in olist if ("LIMIT" in (o.get("order_type") or "")) and (o.get("action") == "SELL")]
        if stop_orders and tp_orders and float(orphan_candidates.get(symbol, 0) or 0) <= 0:
            inferred_qty = max(float(stop_orders[0].get("qty") or 0), float(tp_orders[0].get("qty") or 0))
            if inferred_qty > 0:
                orphan_candidates[symbol] = inferred_qty

    for symbol, qty in list(orphan_candidates.items()):
        ...
        open_orders = open_orders_by_symbol.get(symbol) or _extract_open_orders_for_symbol(client, account_id_key, symbol)
...
def _reserve_pool(state: Dict[str, Any], dollars: float, max_pool: float) -> bool:
    current = _refresh_pool_reserved(state)
    if current + float(dollars or 0.0) > max_pool:
        return False
    state["pool_reserved"] = float(current + float(dollars or 0.0))
...
def _release_pool(state: Dict[str, Any], dollars: float) -> None:
    try:
        state["pool_reserved"] = max(0.0, float(state.get("pool_reserved", 0.0) or 0.0) - float(dollars or 0.0))
    except Exception:
        state["pool_reserved"] = 0.0
    _refresh_pool_reserved(state)
    _assert_pool_invariants(state)
...
def stage_from_payload(cfg: AutoExecConfig, engine: str, payload: Dict[str, Any], stage: str = "STAGED") -> Optional[TradeLifecycle]:
    ...
    if engine == "RIDE":
        ...
        # To ensure PRE vs CONF desired_entry refinement actually triggers, we search:
        #   1) top-level payload
        #   2) payload["Extras"]
        #   3) payload["__raw__"] (best-effort)
...
def reconcile_and_execute(
    ...
    # Post-pass cleanup sweep: broker-flat truth should eventually win over stale stage/order metadata.
    try:
        for symbol, lst in list((state.get("lifecycles") or {}).items()):
            for idx, raw in enumerate(list(lst or [])):
                lc = lifecycle_from_raw(raw)
                stg = str(lc.stage or "").upper()
                if stg in {"CLOSED", "CANCELED", "STAGED", "PRESTAGED"}:
                    lst[idx] = asdict(lc)
                    continue
                try:
                    pos_qty = int(client.get_positions_map(account_id_key).get(str(symbol).upper(), 0) or 0)
                except Exception:
                    pos_qty = 0
                try:
                    ...
                    od = client.list_orders(account_id_key, status="OPEN", count=50, symbol=symbol, from_date=from_s, to_date=to_s)
                    orders = od.get("OrdersResponse", {}).get("Orders", {}).get("Order", []) if isinstance(od, dict) else []
                    if isinstance(orders, dict):
                        orders = [orders]
                    has_open = any(str(o.get("symbol", o.get("Symbol", "")) or "").upper().strip() == str(symbol).upper().strip() for o in (orders or []))
                except Exception:
                    has_open = True
                if pos_qty == 0 and not has_open:
                    if stg == "ENTRY_SENT":
                        oid = _oid_int(getattr(lc, "entry_order_id", None))
                        inactive = False
                        if oid is not None:
                            try:
                                _st, _fq = client.get_order_status_and_filled_qty(account_id_key, int(oid), symbol=symbol)
                                inactive = str(_st or "UNKNOWN").upper() in FINAL_STATUSES
                            except Exception:
                                inactive = False
                        if inactive:
                            ...
                            lc.stage = "CANCELED"
                            lc.notes = (lc.notes or "") + " | postpass_entry_flat_finalize"
                    elif stg in {"IN_POSITION", "CANCEL_PENDING", "EOD_PENDING"}:
                        if float(lc.reserved_dollars or 0.0) > 0.0:
                            _release_pool(state, float(lc.reserved_dollars or 0.0))
                            lc.reserved_dollars = 0.0
                        try:
                            _record_realized_trade_on_close(state, lc, client, account_id_key, "POSTPASS_BROKER_FLAT")
                        except Exception:
                            pass
                        lc.entry_order_id = None
                        ...
                        lc.stage = "CLOSED"
                        lc.notes = (lc.notes or "") + " | postpass_broker_flat_finalize"
                lst[idx] = asdict(lc)
        _refresh_pool_reserved(state)
        st.session_state["autoexec"] = state
    except Exception:
...
def try_send_entries(cfg: AutoExecConfig, allow_opening: bool, allow_midday: bool, allow_power: bool, fetch_last_price_fn) -> None:
    ...
    def _entry_recovery_search_window(ref_now) -> tuple[str, str]:
        """Return a recent ET date window for duplicate/in-flight entry recovery."""
        try:
            ref_et = ref_now.astimezone(ET)
        except Exception:
            ref_et = ref_now
        return ((ref_et.date() - timedelta(days=2)).isoformat(), ref_et.date().isoformat())
        ...
        for idx, raw in enumerate(list(lst)):
            ...
            try:
                ...
                if recent_submit:
                    recovered = None
                    try:
                        _from_date, _to_date = _entry_recovery_search_window(now)
                        recovered = client.find_order_by_client_order_id(
                            account_id_key=account_id_key,
            ...
            if entry_mode == "immediate_on_stage":
                ...
                if not (last <= lifecycle.desired_entry and last > lifecycle.stop):
                    # 45s-rerun safety: on the very first STAGED evaluation, allow a resting
                    # limit order at desired_entry when price is still above entry but not yet
                    # stretched beyond TP0. This avoids missing fast pullbacks that occur
                    # between reruns while keeping us out of already-overextended tape.
                    allow_resting_fallback = False
                    try:
                        first_eval = int(getattr(lifecycle, "entry_eval_count", 0) or 0) <= 1
                    except Exception:
                        first_eval = False
                    tp0_px = None
                    try:
                        tp0_px = float(lifecycle.tp0) if lifecycle.tp0 is not None else None
                    except Exception:
                        tp0_px = None
                    try:
                        desired_px = float(lifecycle.desired_entry)
                        stop_px = float(lifecycle.stop)
                    except Exception:
                        desired_px = None
                        stop_px = None
                    if first_eval and desired_px is not None and stop_px is not None:
                        try:
                            allow_resting_fallback = bool(last > desired_px and last > stop_px and (tp0_px is None or float(last) <= float(tp0_px)))
                        except Exception:
                            allow_resting_fallback = False
                    if not allow_resting_fallback:
                        lifecycle.last_entry_eval = (
                        ...
                        continue
                    lifecycle.last_entry_eval = (
                        f"ALLOW: touch_resting_fallback last={last} stop={lifecycle.stop} desired={lifecycle.desired_entry} tp0={lifecycle.tp0}"
                    )
                    _append_note(lifecycle, f"ENTRY_TOUCH_FALLBACK last={last} desired={lifecycle.desired_entry} tp0={lifecycle.tp0}")
...
def _reconcile_one(client: ETradeClient, account_id_key: str, state: Dict[str, Any], lifecycle: TradeLifecycle, cfg: AutoExecConfig, fetch_last_price_fn=None) -> None:
    ...
    def _get_broker_position_qty(symbol: str, *, force_refresh: bool = False) -> Optional[int]:
        ...
        pos_qty = None
        try:
            pos_map = _get_positions_map_cached(ttl_sec=(0 if force_refresh else 15)) or {}
        except Exception:
            pos_map = {}
    ...
    def _known_orders_inactive(order_ids: list[Optional[str]]) -> bool:
        had_any = False
        for order_id_str in (order_ids or []):
            oid = _oid_int_safe(order_id_str)
            if oid is None:
                continue
            had_any = True
            if not _order_is_inactive(oid):
                return False
        return had_any

    def _symbol_has_open_orders(symbol: str) -> bool:
        try:
            ...
            for o in (orders or []):
                sym = str(o.get("symbol", o.get("Symbol", "")) or "").upper().strip()
                if sym == str(symbol).upper().strip():
                    return True
        except Exception:
            return True
        return False

    def _finalize_flat_terminal(reason: str, *, terminal_stage: str = "CLOSED") -> None:
        if float(lifecycle.reserved_dollars or 0.0) > 0.0:
            _release_pool(state, float(lifecycle.reserved_dollars or 0.0))
            lifecycle.reserved_dollars = 0.0
        try:
            if terminal_stage == "CLOSED":
        ...
        lifecycle.stop_submit_started_at = None
        lifecycle.stop_client_order_id = None
        lifecycle.tp0_submit_started_at = None
        lifecycle.tp0_client_order_id = None
        lifecycle.stage = terminal_stage
        lifecycle.notes = (lifecycle.notes or "") + f" | flat_finalize:{reason}"
        try:
            _set_protection_state(lifecycle, "NONE", qty=0, err=None)
        except Exception:
            pass
        _event_once(
            cfg,
            lifecycle,
            f"FLAT_FINALIZE_{terminal_stage}_{reason}",
            f"[AUTOEXEC] {lifecycle.symbol} {terminal_stage}",
            f"""Time (ET): {now.isoformat()}
Symbol: {lifecycle.symbol}
Engine: {lifecycle.engine}

Finalized from broker-flat truth.
Reason: {reason}
...
""",
        ...
        # Orphan/lag cleanup: if broker shows no position and all known entry-side orders are inactive,
        # do not let the lifecycle sit in ENTRY_SENT indefinitely.
        try:
            entry_age_sec = max(0.0, (now - sent).total_seconds())
        except Exception:
            entry_age_sec = 0.0
        if force_filled_qty == 0 and entry_age_sec >= 45:
            try:
                broker_qty_now = int(_get_broker_position_qty(lifecycle.symbol, force_refresh=True) or 0)
            except Exception:
                broker_qty_now = 0
            if broker_qty_now <= 0:
                known_inactive = _known_orders_inactive([lifecycle.entry_order_id])
                broker_has_open = _symbol_has_open_orders(lifecycle.symbol)
                if known_inactive and not broker_has_open:
                    _finalize_flat_terminal("ENTRY_ORPHAN_RECOVERED", terminal_stage="CANCELED")
                    return
...
""",
            ...
            if lifecycle.stop_order_id:
                ...
                _set_protection_state(lifecycle, "FAILED", qty=int(filled_qty), err="missing=STOP")
...
""",
        ...
        pos_qty = None
        try:
            pos_qty = _get_broker_position_qty(lifecycle.symbol)
        except Exception:
            pos_qty = None

        tracked_position_qty = max(int(bracket_qty or 0), int(lifecycle.filled_qty or 0), int(getattr(lifecycle, "protection_required_qty", 0) or 0))
        if tracked_position_qty > 0 and pos_qty is not None:
            # Fully flat at broker -> close lifecycle even if order-status is UNKNOWN.
            if pos_qty == 0:
                inferred = None
                # Try to infer which leg executed for reporting/cleanup.
                try:
                    if lifecycle.tp0_order_id:
                        ...
                        if _toid is not None:
                            try:
                                ...
                                _ts, _tf = ("UNKNOWN", 0)
                            if str(_ts).upper() in {"EXECUTED", "FILLED"} or (_tf and int(_tf) >= tracked_position_qty):
                    ...
                    if inferred is None and lifecycle.stop_order_id:
                        ...
                        if _soid is not None:
                            try:
                                ...
                                _ss, _sf = ("UNKNOWN", 0)
                            if str(_ss).upper() in {"EXECUTED", "FILLED"} or (_sf and int(_sf) >= tracked_position_qty):
                ...
                # Safety cleanup: cancel any remaining bracket leg orders best-effort.
                try:
                    if lifecycle.stop_order_id:
                        _soid = _oid_int(lifecycle.stop_order_id)
                        if _soid is not None:
                ...
                _close(inferred or "CLOSED_POSITION_ZERO_UNKNOWN")
                return

            # Partial exit at broker -> enforce "close fully" by flattening remainder.
            if 0 < pos_qty < tracked_position_qty:
                ...
                _close("POSITION_PARTIAL_FLATTEN")
                return

        # --- Unprotected-position guard ---
        live_qty = 0
        try:
            live_qty = int(pos_qty if pos_qty is not None else (bracket_qty or lifecycle.filled_qty or 0))
        except Exception:
            live_qty = int(bracket_qty or lifecycle.filled_qty or 0)
        if live_qty > 0:
            lifecycle.protection_required_qty = int(live_qty)
        try:
            last_px_guard = _parse_float(fetch_last_price_fn(lifecycle.symbol)) if fetch_last_price_fn is not None else None
        except Exception:
            last_px_guard = None
        if live_qty > 0 and not lifecycle.stop_order_id and not lifecycle.market_exit_order_id:
            _set_protection_state(lifecycle, "FAILED", qty=int(live_qty), err=getattr(lifecycle, "protection_last_error", None) or "stop_missing")
            breach_now = False
            try:
                breach_now = last_px_guard is not None and lifecycle.stop is not None and float(last_px_guard) <= float(lifecycle.stop)
            except Exception:
                breach_now = False
            fail_count = int(getattr(lifecycle, "protection_fail_count", 0) or 0)
            should_emergency_flatten = breach_now or fail_count >= 3
            if should_emergency_flatten:
                reason = "UNPROTECTED_STOP_BREACH" if breach_now else "UNPROTECTED_STOP_RETRY_EXHAUSTED"
                if not getattr(lifecycle, "emergency_flatten_started_at", None):
                    lifecycle.emergency_flatten_started_at = now.isoformat()
                    lifecycle.emergency_flatten_reason = reason
                _event_once(
                    cfg,
                    lifecycle,
                    f"EMERGENCY_FLATTEN_{reason}",
                    f"[AUTOEXEC] {lifecycle.symbol} EMERGENCY FLATTEN",
                    f"""Time (ET): {now.isoformat()}
Symbol: {lifecycle.symbol}
Engine: {lifecycle.engine}

Emergency flatten because protective stop is unavailable.
Reason: {reason}
Live qty: {live_qty}
Last: {last_px_guard}
Stop: {lifecycle.stop}
""",
                ...
                try:
                    mcoid = _mk_client_order_id(lifecycle.lifecycle_id, 'MKUF')
                    if not _close_inflight(lifecycle, now, mcoid, 90):
                        _mark_close_submit_started(lifecycle, now, mcoid)
                        close_mode, close_oid, _ppid = _place_sell_close_best_effort(
                            client=client,
                            account_id_key=account_id_key,
                            symbol=lifecycle.symbol,
                            qty=int(live_qty),
                            client_order_id=mcoid,
                            last_px=last_px_guard,
                            lifecycle=lifecycle,
                            note_prefix="MK_UNPROTECTED",
                        )
                        lifecycle.market_exit_order_id = str(close_oid) if close_oid is not None else None
                        lifecycle.notes = (lifecycle.notes or "") + f" | emergency_flatten_submitted:{reason}"
                except Exception as e:
                    lifecycle.notes = (lifecycle.notes or "") + f" | emergency_flatten_error:{type(e).__name__}:{str(e)[:120]}"
                    return
                return

        # --- Time-based profit capture (optional) ---
        # If TP0 has not triggered after N minutes but the trade is up by >= threshold%, exit to lock profit.
        try:
            if bool(getattr(cfg, "enable_time_profit_capture", False)) and not getattr(lifecycle, "tp0_triggered_at", None) and not lifecycle.tp0_order_id:
                # Determine entry timestamp (prefer broker-detected execution time)
                ...
                if entry_dt is not None:
                    mins = float(getattr(cfg, "time_profit_capture_minutes", 12) or 0)
                    if (now - entry_dt).total_seconds() >= mins * 60.0:
                        # Determine entry price (prefer broker avg price)
                        entry_px = None
                        ...
                        # Determine last price (use provided last-price fetcher)
                        last_px = None
                        ...
                        if entry_px and entry_px > 0 and last_px is not None:
                            gain_pct = (float(last_px) - float(entry_px)) / float(entry_px) * 100.0
                            thresh = float(getattr(cfg, "time_profit_capture_profit_pct", 0.5) or 0.0)
                            if gain_pct >= thresh:
                                # Close fully: cancel STOP (best effort) then sell remaining position.
                                try:
                                    if lifecycle.stop_order_id:
                                        ...
                                        if soid is not None:
                                            try:
                                                ...
                                                _append_note(lifecycle, f"TIME_PROFIT_CAPTURE_STOP_CANCEL_OK oid={soid}")
                                            except Exception as ce:
                                                _append_note(lifecycle, f"TIME_PROFIT_CAPTURE_STOP_CANCEL_ERR {str(ce)[:180]}")
                                except Exception:
                                    pass
                                finally:
                                    lifecycle.stop_order_id = None

                                # Determine remaining qty from broker positions (truth)
                                rem_qty = 0
                                try:
                                ...
                                try:
                                    if rem_qty > 0:
                                        ...
                                        if _close_inflight(lifecycle, now, close_coid, 90):
                                            _append_note(lifecycle, 'TIME_PROFIT_CAPTURE_SKIP_DUP')
                                            return
                                        ...
                                        close_mode, close_oid, _ppid = _place_sell_close_best_effort(
                                            ...
                                            lifecycle=lifecycle,
                                            reason_tag="TIME_PROFIT_CAPTURE",
                                        )
                                        if close_oid is not None:
                                            lifecycle.market_exit_order_id = str(close_oid)
                                except Exception as pc_exc:
                                    lifecycle.notes = (lifecycle.notes or "") + f" | time_profit_capture_error:{pc_exc}"
                                    # Re-establish STOP protection if we canceled it but could not place the closing order.
                                    try:
                                        _ensure_brackets(client, account_id_key, lifecycle.symbol, lifecycle, cfg, log_prefix="[AUTOEXEC] ")
                                    except Exception as re_exc:
                                        lifecycle.notes = (lifecycle.notes or "") + f" | time_profit_capture_reprotect_error:{re_exc}"
                                    return

                                lifecycle.notes = (lifecycle.notes or "") + f" | TIME_PROFIT_CAPTURE gain_pct={gain_pct:.2f}% mins={mins:.1f} mode={close_mode}"
                                _close("TIME_PROFIT_CAPTURE")
                                return
        except Exception as e:
            lifecycle.notes = (lifecycle.notes or "") + f" | time_profit_capture_error:{e}"

        # --- Synthetic TP0 trigger (no resting TP0 order) ---
        # E*TRADE may reserve shares for an open SELL order and reject a second "closing" SELL with code=1037.
        # To avoid that, we keep the protective STOP live, and only place TP0 when price reaches the TP0 trigger.
        try:
            ...
            # If TP0 was triggered and a TP0 order is still working too long, fail-safe flatten at market.
            tp0_trig_ts = getattr(lifecycle, 'tp0_triggered_at', None)
            if tp0_trig_ts and lifecycle.tp0_order_id:
                try:
                    dt = datetime.fromisoformat(str(tp0_trig_ts))
                except Exception:
                    dt = None
                if dt is not None and (now - dt).total_seconds() >= 90:
                    # If still in position, cancel TP0 and market-sell remainder (close fully).
                    try:
                        ...
                        if toid is not None:
                            ...
                            if str(t_status).upper() not in {"EXECUTED", "FILLED"}:
                                try:
                    ...
                    try:
                        ...
                        rem = 0
                    tp0_timeout_last_px = None
                    if fetch_last_price_fn is not None:
                        try:
                            tp0_timeout_last_px = _parse_float(fetch_last_price_fn(tp_symbol or lifecycle.symbol))
                        except Exception:
                            tp0_timeout_last_px = None
                    if rem > 0:
                        try:
                            mcoid = _mk_client_order_id(lifecycle.lifecycle_id, 'MKTP0')
                            if _close_inflight(lifecycle, now, mcoid, 90):
                                _append_note(lifecycle, 'MK_TP0_TIMEOUT_SKIP_DUP')
                            else:
                                _mark_close_submit_started(lifecycle, now, mcoid)
                                mode, mid, mpid = _place_sell_close_best_effort(
                                    client=client,
                                    ...
                                    qty=int(rem),
                                    client_order_id=mcoid,
                                    last_px=tp0_timeout_last_px,
                                    lifecycle=lifecycle,
                                    note_prefix="MK_TP0_TIMEOUT",
                                )
                                lifecycle.market_exit_order_id = str(mid) if mid is not None else None

                        except Exception:
                            pass
                    _append_note(lifecycle, "TP0_TRIGGER_TIMEOUT_MARKET")
                    _close("TP0_TRIGGER_TIMEOUT_MARKET")
                    return

            # Only attempt trigger if we have a TP0 level and no TP0 order currently working.
            if tp0_px is not None and not lifecycle.tp0_order_id:
                # Respect a small backoff between trigger attempts.
                ok_attempt = True
                ts = getattr(lifecycle, 'tp0_last_attempt_at', None)
                if ts:
                    try:
                        dt = datetime.fromisoformat(str(ts))
                    except Exception:
                        dt = None
                    if dt is not None and (now - dt).total_seconds() < 30:
                        ok_attempt = False

                # Fetch last price (best-effort). If we can't fetch, we can't trigger.
                last_px = None
                if fetch_last_price_fn is not None:
                    try:
                        last_px = _parse_float(fetch_last_price_fn(lifecycle.symbol))
                ...
                if ok_attempt and last_px is not None and last_px >= tp0_px:
                    lifecycle.tp0_last_attempt_at = now.isoformat()
                    lifecycle.tp0_triggered_at = getattr(lifecycle, 'tp0_triggered_at', None) or now.isoformat()
                    _append_note(lifecycle, f"TP0_TRIGGER_FIRED last={last_px} tp0={tp0_px}")

                    # Cancel STOP to free shares for the closing LIMIT (synthetic bracket).
                    if lifecycle.stop_order_id:
                        soid = _oid_int(lifecycle.stop_order_id)
                        if soid is not None:
                            try:
                                client.cancel_order(account_id_key, soid)
                                _append_note(lifecycle, f"STOP_CANCEL_FOR_TP0_TRIGGER_OK oid={soid}")
                            except Exception as ce:
                                _append_note(lifecycle, f"STOP_CANCEL_FOR_TP0_TRIGGER_ERR {str(ce)[:160]}")
                        lifecycle.stop_order_id = None

                    # Place TP0 as a closing LIMIT for full bracket_qty.
                    tp_coid = str(lifecycle.tp0_client_order_id or _mk_client_order_id(lifecycle.lifecycle_id, 'TP'))
                    lifecycle.tp0_client_order_id = tp_coid
                    if _recent_submit(getattr(lifecycle, 'tp0_submit_started_at', None), now, 90):
                        _append_note(lifecycle, 'TP0_SUBMIT_INFLIGHT<90s')
                    else:
                        lifecycle.tp0_submit_started_at = now.isoformat()
                        try:
                            tp_oid, tp_pid = client.place_equity_limit_order_ex(
                                ...
                                action='SELL',
                                qty=int(bracket_qty),
                                limit_price=float(_tick_round(tp0_px) or tp0_px),
                            ...
                            emsg = str(te)
                            _append_note(lifecycle, f"TP0_PLACE_ERR {emsg[:220]}")
                            recovered = None
                            try:
                                recovered = client.find_order_by_client_order_id(
                                    account_id_key=account_id_key,
                                ...
                                recovered = None
                            if recovered and recovered.get('orderId'):
                                lifecycle.tp0_order_id = str(recovered.get('orderId'))
                                _append_note(lifecycle, f"TP0_ATTACH_DUP_GUARD oid={lifecycle.tp0_order_id}")
        except Exception:
            pass
...
def _force_liquidate_all(client: ETradeClient, account_id_key: str, cfg: AutoExecConfig, state: Dict[str, Any]) -> None:
    """Hard liquidation by 15:55 ET using broker-truth confirmation."""
    now = _now_et()

    def _managed_pos_qty(sym: str) -> int:
        try:
            pos_map = client.get_positions_map(account_id_key) or {}
            return int(float((pos_map or {}).get(str(sym).upper(), 0) or 0))
        except Exception:
            return 0

    for symbol, lst in list((state.get("lifecycles") or {}).items()):
        new_lst = []
        for raw in list(lst or []):
            lc = lifecycle_from_raw(raw)
            if lc.stage in {"CLOSED", "CANCELED"}:
                new_lst.append(asdict(lc))
                continue

            lc.eod_liquidation_started_at = lc.eod_liquidation_started_at or now.isoformat()
            eod_cancel_targets = {
                "entry": lc.entry_order_id,
                "stop": lc.stop_order_id,
                "tp0": lc.tp0_order_id,
            }
            for _lbl, _oid_str in list(eod_cancel_targets.items()):
                oid_i = _oid_int(_oid_str)
                if oid_i is not None:
                    try:
                        client.cancel_order(account_id_key, oid_i)
                    except Exception:
            ...
            broker_qty = _managed_pos_qty(symbol)
            if broker_qty > 0:
                try:
                    mcoid = lc.close_client_order_id or _mk_client_order_id(lc.lifecycle_id, 'EODMK')
                    if not _close_inflight(lc, now, mcoid, 120):
                        _mark_close_submit_started(lc, now, mcoid)
                        _mode, mid, _mpid = _place_sell_close_best_effort(
                            client=client,
                            account_id_key=account_id_key,
                            symbol=symbol,
                            qty=int(broker_qty),
                        ...
                        lc.market_exit_order_id = str(mid) if mid is not None else None
                except Exception as e:
                    _append_note(lc, f"EOD_MARKET_EXIT_ERR {type(e).__name__}:{str(e)[:160]}")

            broker_qty_after = _managed_pos_qty(symbol)
            active_orders = False
            for oid in [eod_cancel_targets.get("entry"), eod_cancel_targets.get("stop"), eod_cancel_targets.get("tp0"), lc.market_exit_order_id]:
                oid_i = _oid_int(oid)
                if oid_i is None:
                    continue
                try:
                    ost, _fq = client.get_order_status_and_filled_qty(account_id_key, int(oid_i), symbol=symbol)
                    if str(ost).upper() in {"EXECUTED", "FILLED", "CANCELLED", "REJECTED", "EXPIRED"}:
                        continue
                except Exception:
                    pass
                try:
                    if not _order_is_inactive(int(oid_i)):
                        active_orders = True
                        break
                except Exception:
                    active_orders = True
            ...
            if broker_qty_after <= 0 and not active_orders:
                if float(lc.reserved_dollars or 0.0) > 0.0:
                    lc.reserved_dollars = 0.0
                try:
                    _record_realized_trade_on_close(state, lc, client, account_id_key, "EOD_LIQUIDATION")
                except Exception:
                    pass
                lc.entry_order_id = None
                lc.stop_order_id = None
                ...
                lc.market_exit_order_id = None
                lc.stage = "CLOSED"
                lc.notes = (lc.notes or "") + " | EOD_LIQUIDATION_CONFIRMED"
                _event_once(
                    cfg,
                    lc,
                    "EOD_LIQUIDATION_CONFIRMED",
                    f"[AUTOEXEC] {lc.symbol} EOD LIQUIDATION CONFIRMED",
                    f"""Time (ET): {now.isoformat()}
Symbol: {lc.symbol}
Engine: {lc.engine}

Broker confirmed flat after EOD liquidation.
""",
                )
            else:
                lc.stage = "EOD_PENDING"
                lc.cancel_requested_at = lc.cancel_requested_at or now.isoformat()
                lc.notes = (lc.notes or "") + " | pending_close_reason:EOD_LIQUIDATION | pending_close"
                _event_once(
                    cfg,
                    lc,
                    "EOD_LIQUIDATION_PENDING",
                    f"[AUTOEXEC] {lc.symbol} EOD LIQUIDATION PENDING",
                    f"""Time (ET): {now.isoformat()}
Symbol: {lc.symbol}
Engine: {lc.engine}

EOD liquidation submitted; waiting for broker-confirmed flat position.
Broker qty: {broker_qty_after}
""",
                )
            new_lst.append(asdict(lc))
